...
// MARK: Async for lat+lon
    ...
    // MARK: Async for city name
    
        func loadWeatherForCityName(_ name: String) async throws -> Result<ResponseBody, WeatherAPIError> {
            guard let url = URL(string: "https://api.openweathermap.org/data/2.5/weather?q=\(name.capitalized),ru&appid=\(key)&units=metric")
            else {
                return Result.failure(.requestFailed(description: "Wrong URL"))
            }
            let urlRequest = URLRequest(url: url)
